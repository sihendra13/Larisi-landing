...
for file in glob.glob('blog/*.html'):
    ...
    # Replace X (Twitter) back to Threads with the circle SVG
    def replacer(match):
        url = match.group(1)
        encoded_title = match.group(2)
        return f'<a href="https://www.threads.net/intent/post?text={encoded_title}%20-%20{url}" target="_blank" class="share-btn th" title="Bagikan ke Threads">\n                {th_circle_svg}\n            </a>'
    
    content = re.sub(
        r'<a href="https://twitter\.com/intent/tweet\?url=(.*?)&text=(.*?)" target="_blank" class="share-btn th" title="Bagikan ke X \(Twitter\)">\s*<svg.*?</svg>\s*</a>',
        replacer, 
        content, 
        flags=re.DOTALL
    )
...
print("Restored Threads logo (circular version).")
